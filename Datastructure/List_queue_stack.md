...
1. 循环队列的简单实现及判别条件
   ...
   有一点需要注意，循环队列的空间利用率并不是100%，试想如果是完全利用率，队列空时头指针与尾指针是相等的，当我将数据存满MaxSize，队列的首尾指针再次相等了，这无法作为判别队列状态的条件，因此需要牺牲一个位置。即元素入队时，通常约定rear指针先向前移动一步，再从移动后的位置来做增添操作；在出队列时，则需要front指针前移一步，无需额外操作。用这套规则，在判断队满时只需判别rear指针再移动一个位置是否与front指针相等即可。
